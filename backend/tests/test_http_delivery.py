...
from app.modules.fleet.infrastructure.orm import Delivery, SchoolReceiving
...
@pytest.mark.parametrize('school_case', ['accepted', 'expired_rejection', 'discard', 'late_consumption'])
async def test_delivery_business_flow(school_case):
        ...
        async with engine.connect() as c:
            ...
            try:
                ...
                async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url='http://test') as client:
                    ...
                    await c.execute(text('RESET ROLE'))
                    driver, vehicle, driver2, vehicle2, foreign_school = (uuid4() for _ in range(5))
                    for d, v, code in ((driver, vehicle, 'ONE'), (driver2, vehicle2, 'TWO')):
                        await c.execute(insert(Driver).values(driver_id=d, tenant_id=seed_id('tenant'), driver_code=code, driver_name='Example driver'))
                        await c.execute(insert(Vehicle).values(vehicle_id=v, tenant_id=seed_id('tenant'), vehicle_code=code, plate_number=code, vehicle_type='VAN', driver_id=d))
                    await c.execute(insert(School).values(school_id=foreign_school, tenant_id=other, kitchen_id=foreign_kitchen, school_code='FOREIGN', school_name='Foreign school'))
                    foreign_driver, foreign_vehicle, foreign_delivery = uuid4(), uuid4(), uuid4()
                    await c.execute(insert(Driver).values(driver_id=foreign_driver, tenant_id=other, driver_code='FOREIGN', driver_name='Foreign'))
                    await c.execute(insert(Vehicle).values(vehicle_id=foreign_vehicle, tenant_id=other, vehicle_code='FOREIGN', plate_number='FOREIGN', vehicle_type='VAN'))
                    await c.execute(insert(Delivery).values(delivery_id=foreign_delivery, tenant_id=other, kitchen_id=foreign_kitchen, driver=foreign_driver, vehicle=foreign_vehicle, status='CANCELLED'))

                    await c.execute(text('SET LOCAL ROLE fsos_runtime'))
                    deliveries = '/api/v1/deliveries'
                    manifest = {'kitchen_id': str(seed_id('kitchen')), 'vehicle': str(vehicle), 'driver': str(driver), 'items': [{'package_id': pkid, 'school_id': str(seed_id('school')), 'expected_version': 3}]}
                    assert (await client.get(deliveries)).status_code == 401
                    assert (await client.post(deliveries, headers=headers, json={**manifest, 'driver': str(driver2)})).status_code == 409
                    assert (await client.post(deliveries, headers=headers, json={**manifest, 'items': [{**manifest['items'][0], 'school_id': str(foreign_school)}]})).status_code == 409
                    assert (await client.post(deliveries, headers=headers, json={**manifest, 'items': manifest['items'] * 2})).status_code == 400
                    bad_manifest = {**manifest, 'items': manifest['items'] + [{'package_id': str(uuid4()), 'school_id': str(seed_id('school')), 'expected_version': 1}]}
                    assert (await client.post(deliveries, headers=headers, json=bad_manifest)).status_code == 409
                    assert (await client.get(f'{packages}/{pkid}', headers=headers)).json()['data']['version'] == 3
                    created_delivery = await client.post(deliveries, headers=headers, json=manifest)
                    assert created_delivery.status_code == 201, created_delivery.text
                    first = created_delivery.json()['data']; first_id = first['delivery_id']
                    assert first['status'] == 'CREATED' and first['items'][0]['package']['status'] == 'ALLOCATED'
                    assert first['items'][0]['package']['version'] == 4
                    assert (await client.post(deliveries, headers=headers, json=manifest)).status_code == 409
                    assert (await client.post(deliveries, headers=headers, json={**manifest, 'vehicle': str(vehicle2), 'driver': str(driver2), 'items': [{**manifest['items'][0], 'expected_version': 4}]})).status_code == 409
                    assert (await client.post(f'{packages}/{pkid}/holding/update', headers=headers, json={'expected_version': 4})).status_code == 409
                    cancelled = await client.post(f'{deliveries}/{first_id}/cancel', headers=headers, json={'expected_version': 1})
                    assert cancelled.status_code == 200, cancelled.text
                    assert cancelled.json()['data']['items'][0]['package']['status'] == 'RELEASED'
                    assert cancelled.json()['data']['items'][0]['package']['version'] == 5
                    created_delivery = await client.post(deliveries, headers=headers, json={**manifest, 'items': [{**manifest['items'][0], 'expected_version': 5}]})
                    assert created_delivery.status_code == 201, created_delivery.text
                    did = created_delivery.json()['data']['delivery_id']
                    depart_url = f'{deliveries}/{did}/depart'
                    assert (await client.post(depart_url, headers=headers, json={'expected_version': 1, 'estimated_arrival_time': package['expired_at']})).status_code == 409
                    assert (await client.post(depart_url, headers=headers, json={'expected_version': 1, 'estimated_arrival_time': '2099-01-01T00:00:00'})).status_code == 400
                    eta = (datetime.now(UTC) + timedelta(minutes=10)).isoformat()
                    departed = await client.post(depart_url, headers=headers, json={'expected_version': 1, 'estimated_arrival_time': eta})
                    assert departed.status_code == 200, departed.text
                    trip = departed.json()['data']
                    assert trip['status'] == 'IN_TRANSIT' and trip['version'] == 2
                    assert trip['items'][0]['package']['status'] == 'IN_TRANSIT'
                    assert (await client.post(depart_url, headers=headers, json={'expected_version': 1, 'estimated_arrival_time': eta})).status_code == 409
                    assert (await client.post(f'{deliveries}/{did}/cancel', headers=headers, json={'expected_version': 2})).status_code == 409
                    # Arrival records reality even when holding time expired in transit.
                    class Later(datetime):
                        @classmethod
                        def now(cls, tz=None):
                            return (datetime.fromisoformat(package['expired_at']) + timedelta(seconds=1)
                                    if school_case == 'expired_rejection' else datetime.now(UTC))
                    with patch('app.modules.fleet.application.delivery_service.datetime', Later):
                        arrived = await client.post(f'{deliveries}/{did}/complete', headers=headers, json={'expected_version': 2})
                        assert arrived.status_code == 200, arrived.text
                        done_delivery = arrived.json()['data']
                        assert done_delivery['status'] == 'COMPLETED' and done_delivery['version'] == 3
                        assert done_delivery['items'][0]['package']['status'] == 'DELIVERED'
                        assert done_delivery['items'][0]['package']['effective_status'] == ('EXPIRED' if school_case == 'expired_rejection' else 'DELIVERED')
                        assert done_delivery['items'][0]['package']['holding_eligible'] is False
                    assert (await client.post(f'{deliveries}/{did}/complete', headers=headers, json={'expected_version': 3})).status_code == 409
                    assert not await c.scalar(select(SchoolReceiving.school_receiving_id).where(SchoolReceiving.package == UUID(pkid)))
                    assert (await client.get(f'{deliveries}/{foreign_delivery}', headers=headers)).status_code == 404
                    assert (await client.get(deliveries, headers=headers, params={'status': 'COMPLETED', 'limit': 1})).json()['data']['items'][0]['delivery_id'] == did
                    p_asset = await c.scalar(select(DigitalAsset.asset_uuid).where(DigitalAsset.entity_uuid == UUID(pkid)))
                    moves = (await c.execute(select(AssetMovement.movement_type).where(AssetMovement.asset_uuid == p_asset))).scalars().all()
                    assert set(moves) == {'PACKAGING', 'VEHICLE_LOADING', 'DELIVERY'}
                    assert len(moves) == 3
                    assert set((await c.execute(select(EventLog.event_type).where(EventLog.entity_uuid == UUID(did)))).scalars()) == {'delivery.created', 'delivery.departed', 'delivery.completed'}
                    receipts, consumptions = '/api/v1/school-receivings', '/api/v1/consumptions'
                    from app.modules.consumption.infrastructure.orm import Consumption
                    assert (await client.get(receipts)).status_code == 401
                    assert (await client.get(consumptions)).status_code == 401
                    receipt_body = {'delivery_id': did, 'package': pkid, 'school': str(seed_id('school')),
                                    'expected_version': 8, 'condition': 'GOOD', 'accepted': True, 'received_quantity': '1.5',
                                    'notes': 'Shortage of 0.5 portion', 'temperature': '45.25', 'photo': 'example/receipt.jpg'}
                    consume_body = {'package_id': pkid, 'expected_version': 9, 'consumed_quantity': '1', 'discarded_quantity': '0.5', 'notes': 'Unused portion discarded'}
                    assert (await client.post(consumptions, headers=headers, json=consume_body)).status_code == 409
                    for bad in ({'accepted': 'true'}, {'received_quantity': True}, {'expected_version': True}, {'condition': 'UNKNOWN'}, {'notes': 'bad\x00value'}):
                        assert (await client.post(receipts, headers=headers, json={**receipt_body, **bad})).status_code == 400
                    for bad in ({'expected_version': 7}, {'school': str(foreign_school)}, {'delivery_id': str(foreign_delivery)},
                                {'condition': 'DAMAGED'}, {'received_quantity': '3'}, {'notes': None}, {'condition': 'MISSING'}):
                        assert (await client.post(receipts, headers=headers, json={**receipt_body, **bad})).status_code == 409
                    if school_case == 'accepted':
                        from unittest.mock import AsyncMock

                        from app.modules.consumption.application.service import (
                            SchoolWorkflowConflictError,
                        )
                        with patch('app.modules.consumption.application.service.SchoolWorkflowService.record_event',
                                   new=AsyncMock(side_effect=SchoolWorkflowConflictError('Simulated event failure'))):
                            assert (await client.post(receipts, headers=headers, json=receipt_body)).status_code == 409
                        rolled_back = (await client.get(f'{packages}/{pkid}', headers=headers)).json()['data']
                        assert rolled_back['version'] == 8 and rolled_back['status'] == 'DELIVERED'
                        assert not await c.scalar(select(SchoolReceiving.school_receiving_id).where(SchoolReceiving.package == UUID(pkid)))
                        assert not await c.scalar(select(AssetMovement.movement_id).where(AssetMovement.asset_uuid == p_asset, AssetMovement.movement_type == 'SCHOOL_RECEIVING'))
                    if school_case == 'expired_rejection':
                        with patch('app.modules.consumption.application.service.datetime', Later):
                            assert (await client.post(receipts, headers=headers, json=receipt_body)).status_code == 409
                            rejected = await client.post(receipts, headers=headers, json={**receipt_body, 'accepted': False, 'notes': 'Holding deadline exceeded'})
                            assert rejected.status_code == 201, rejected.text
                            record = rejected.json()['data']
                            assert record['accepted'] is False and record['timer_status'] in ('EXPIRED', 'DISCARD_RECOMMENDED')
                        assert (await client.post(consumptions, headers=headers, json=consume_body)).status_code == 409
                        assert (await client.get(f'{packages}/{pkid}', headers=headers)).json()['data']['status'] == 'REJECTED'
                    else:
                        received = await client.post(receipts, headers=headers, json=receipt_body)
                        assert received.status_code == 201, received.text
                        record = received.json()['data']
                        assert record['discrepancy_quantity'] == '0.500000' and record['received_quantity'] == '1.500000'
                        assert (await client.get(f'{packages}/{pkid}', headers=headers)).json()['data']['version'] == 9
                        assert (await client.post(consumptions, headers=headers, json={**consume_body, 'consumed_quantity': '2'})).status_code == 409
                        assert (await client.post(consumptions, headers=headers, json={**consume_body, 'notes': None})).status_code == 409
                        if school_case == 'discard':
                            consume_body.update(consumed_quantity='0', discarded_quantity='1.5')
                        from contextlib import nullcontext
                        class AfterExpiry(datetime):
                            @classmethod
                            def now(cls, tz=None):
                                return datetime.fromisoformat(package['expired_at']) + timedelta(seconds=1)
                        with patch('app.modules.consumption.application.service.datetime', AfterExpiry) if school_case == 'late_consumption' else nullcontext():
                            finalized = await client.post(consumptions, headers=headers, json=consume_body)
                            assert finalized.status_code == 201, finalized.text
                            consumption = finalized.json()['data']
                        assert consumption['safe'] == (None if school_case == 'discard' else school_case != 'late_consumption')
                        assert (await client.post(consumptions, headers=headers, json={**consume_body, 'expected_version': 10})).status_code == 409
                        assert (await client.get(f"{consumptions}/{consumption['consumption_id']}", headers=headers)).json()['data'] == consumption
                        assert (await client.get(consumptions, headers=headers, params={'package_id': pkid, 'limit': 1})).json()['data']['items'] == [consumption]
                        final_package = (await client.get(f'{packages}/{pkid}', headers=headers)).json()['data']
                        assert final_package['version'] == 10 and final_package['status'] == ('DISCARDED' if school_case == 'discard' else 'CONSUMED')
                        with patch('app.modules.packaging.application.service.datetime', AfterExpiry):
                            assert (await client.get(f'{packages}/{pkid}', headers=headers)).json()['data']['effective_status'] == final_package['status']
                        assert await c.scalar(select(func.count()).select_from(Consumption).where(Consumption.package_id == UUID(pkid))) == 1
                        event = await c.scalar(select(EventLog.payload).where(EventLog.entity_uuid == UUID(consumption['consumption_id'])))
                        assert event['consumption'] == consumption
                    assert (await client.post(receipts, headers=headers, json={**receipt_body, 'expected_version': 9})).status_code == 409
                    assert (await client.get(f"{receipts}/{record['school_receiving_id']}", headers=headers)).json()['data'] == record
                    assert (await client.get(receipts, headers=headers, params={'delivery_id': did, 'school': str(seed_id('school')), 'package_id': pkid, 'limit': 1})).json()['data']['items'] == [record]
                    for endpoint in (receipts, consumptions):
                        assert (await client.get(endpoint, headers=headers, params={'package_id': str(foreign)})).json()['data']['items'] == []
                        assert (await client.get(endpoint, headers=headers, params={'limit': 0})).status_code == 400
                        assert (await client.get(f'{endpoint}/{foreign}', headers=headers)).status_code == 404
                        assert (await client.get(endpoint, headers=headers)).headers['Cache-Control'] == 'no-store'
                    event = await c.scalar(select(EventLog.payload).where(EventLog.entity_uuid == UUID(record['school_receiving_id'])))
                    assert event['school_receiving'] == record
                    assert (await client.post(f'{packages}/{pkid}/holding/update', headers=headers, json={'expected_version': 10})).status_code == 409
                    assert await c.scalar(select(func.count()).select_from(AssetMovement).where(AssetMovement.asset_uuid == p_asset, AssetMovement.movement_type == 'SCHOOL_RECEIVING')) == 1
                    await c.execute(text('RESET ROLE'))
                    for table in ('school_receiving', 'consumption'):
                        if table == 'consumption' and school_case == 'expired_rejection':
                            continue
                        with pytest.raises(DBAPIError):
                            async with c.begin_nested():
                                await c.execute(text(f'UPDATE {table} SET version=version'))
                    await c.execute(update(RolePermission).where(RolePermission.permission_id.in_(select(Permission.permission_id).where(Permission.permission_code.in_(['SchoolReceiving.Write', 'Consumption.Write'])))).values(deleted_at=func.now()))
                    await c.execute(text('SET LOCAL ROLE fsos_runtime'))
                    assert (await client.post(receipts, headers=headers, json=receipt_body)).status_code == 403
                    assert (await client.post(consumptions, headers=headers, json=consume_body)).status_code == 403
                    await c.execute(text('RESET ROLE'))
                    with pytest.raises(DBAPIError):
                        async with c.begin_nested():
                            await c.execute(text('UPDATE delivery_item SET version=version'))
                    await c.execute(update(RolePermission).where(RolePermission.permission_id.in_(select(Permission.permission_id).where(Permission.permission_code == 'Delivery.Depart'))).values(deleted_at=func.now()))
                    await c.execute(text('SET LOCAL ROLE fsos_runtime'))
                    assert (await client.post(depart_url, headers=headers, json={'expected_version': 3, 'estimated_arrival_time': eta})).status_code == 403
                    assert (await client.get(deliveries, headers=headers)).status_code == 200
            finally:
...
def test_school_workflow_openapi():
    paths = {p: ops for p, ops in create_app().openapi()['paths'].items() if p.startswith(('/api/v1/school-receivings', '/api/v1/consumptions'))}
    assert sum(len(ops) for ops in paths.values()) == 6
    for ops in paths.values():
        for operation in ops.values():
            assert '422' not in operation['responses']
