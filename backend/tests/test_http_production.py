...
async def test_production_business_flow():
        ...
        async with engine.connect() as c:
            ...
            try:
                ...
                async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url='http://test') as client:
                    ...
                    await c.execute(text('RESET ROLE'))
                    menu = uuid4()
                    await c.execute(insert(FoodItem).values(food_item_id=menu, tenant_id=seed_id('tenant'), food_code='PROD-MENU', food_name='Production menu', uom='portion'))
                    recipe_id = uuid4()
                    ...
                    second = (await client.post(production, headers=headers, json={**body_prod, 'batch_code': 'PROD-002'})).json()['data']
                    # Changed recipe now needs 2kg, exceeding remaining 1.5kg; no writes survive.
                    rejected = await client.post(f"{production}/{second['production_batch_id']}/start", headers=headers, json={'expected_version': 1, 'items': [{**sources['items'][0], 'expected_version': 5, 'quantity': '2'}]})
                    assert rejected.status_code == 409
                    assert (await client.get(stock + '/stock', headers=headers)).json()['data']['issued_quantity'] == '1.000000'
                    cancelled = await client.post(f"{production}/{second['production_batch_id']}/cancel", headers=headers, json={'expected_version': 1})
                    assert cancelled.status_code == 200 and cancelled.json()['data']['status'] == 'CANCELLED'
                    await c.execute(text('RESET ROLE'))
                    with pytest.raises(DBAPIError):
                        async with c.begin_nested():
                            await c.execute(text('UPDATE production_item SET quantity=1'))
                    await c.execute(update(RolePermission).where(RolePermission.permission_id.in_(select(Permission.permission_id).where(Permission.permission_code == 'Production.Start'))).values(deleted_at=func.now()))
                    await c.execute(text('SET LOCAL ROLE fsos_runtime'))
                    assert (await client.post(start_url, headers=headers, json=sources)).status_code == 403
                    assert (await client.get(production, headers=headers)).status_code == 200
            finally:
...
def test_production_openapi():
    schema = create_app().openapi()
    paths = {p: ops for p, ops in schema['paths'].items() if p.startswith('/api/v1/production-batches')}
