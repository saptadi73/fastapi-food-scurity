...
Status 2026-09-11: allowlist `RECEIVING_PERMISSIONS` berisi 26 kode eksplisit pada
`app/core/database/receiving_permissions.py`. Nama helper historis tetap receiving;
aksesnya kini mencakup seluruh alur sampai konsumsi. Permission Read dan aksi
...
Profil PostgreSQL fsos_runtime berbeda dari RBAC ini. Terapkan migrasi head 0022
...
SchoolReceiving/Consumption berlaku dalam tenant; pembatasan user per sekolah
belum tersedia. Bukti immutable tidak memiliki permission Edit/Delete. Grant ini
juga tidak mengaktifkan complaint/recall, ingestion atau subscription realtime.
