"""Explicit receiving grants for an existing development role, using ADMIN_DATABASE_URL."""
import argparse
import asyncio
import json
import sys
...
async def run(args):
    try:
        ...
        async with factory() as session, session.begin():
            result = await provision_receiving_permissions(session, tenant_id=args.tenant, actor_id=args.actor,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--permission', choices=RECEIVING_PERMISSIONS, action='append', required=True)
