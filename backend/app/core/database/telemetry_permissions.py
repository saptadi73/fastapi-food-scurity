...
TELEMETRY_PERMISSIONS = ('Alarm.Read', 'Alarm.Acknowledge', 'DeviceSession.Read', 'DeviceSession.Close')


class TelemetryGrantConflictError(Exception):
    pass


async def provision_telemetry_permissions(session: AsyncSession, *, tenant_id: UUID, actor_id: UUID,
    ...
    if not permissions or len(permissions) != len(set(permissions)) or not set(permissions) <= set(TELEMETRY_PERMISSIONS):
        raise ValueError('Choose unique telemetry permissions explicitly')
    if type(apply) is not bool:
        raise ValueError('Boolean apply required')
    await session.execute(text('SELECT pg_advisory_xact_lock(hashtextextended(:tenant, 54))'), {'tenant': str(tenant_id)})
    ...
    if actor is None or role is None:
        raise TelemetryGrantConflictError('Active actor, tenant and selected tenant role required')
    plan = []
    for code in sorted(permissions):
        ...
        if permission is not None and permission['deleted_at'] is not None:
            raise TelemetryGrantConflictError('Revoked permission must be reviewed separately')
        grant = None
        ...
        if grant is not None and grant['deleted_at'] is not None:
            raise TelemetryGrantConflictError('Revoked grant must be reviewed separately')
