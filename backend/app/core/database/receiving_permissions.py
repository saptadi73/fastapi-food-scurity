...
async def provision_receiving_permissions(session: AsyncSession, *, tenant_id: UUID, actor_id: UUID,
    ...
    if not permissions or len(permissions) != len(set(permissions)) or not set(permissions) <= set(RECEIVING_PERMISSIONS):
        raise ValueError('Choose unique receiving permissions explicitly')
    if type(apply) is not bool:
        raise ValueError('Boolean apply required')
    await session.execute(text('SELECT pg_advisory_xact_lock(hashtextextended(:tenant, 57))'), {'tenant': str(tenant_id)})
    ...
    if actor is None or role is None:
        raise ReceivingGrantConflictError('Active actor, tenant and selected tenant role required')
    plan = []
    for code in sorted(permissions):
        ...
        if permission is not None and permission['deleted_at'] is not None:
            raise ReceivingGrantConflictError('Revoked permission must be reviewed separately')
        grant = None
        ...
        if grant is not None and grant['deleted_at'] is not None:
            raise ReceivingGrantConflictError('Revoked grant must be reviewed separately')
