...
from app.modules.master.application.packaging_service import (
    PackagingConflictError,
    PackagingTypeService,
)


def packaging_dependency(kind):
    async def dependency(db: DatabaseDep,
                         account: Annotated[AuthenticatedAccount, Depends(current_account, scope='function')]):
        try:
            yield PackagingTypeService(db, ActorScope(account.tenant_id, account.user_id), kind)
            ...
            raise HTTPException(404, 'Packaging type not found') from None
        except VersionConflictError:
            raise HTTPException(409, 'Packaging type changed; reload before retrying') from None
        except PackagingConflictError as exc:
            raise HTTPException(409, str(exc)) from None
        except IntegrityError as exc:
            if getattr(exc.orig, 'sqlstate', None) == '23505':
                raise HTTPException(409, 'Packaging type code already exists in this tenant') from None
            if getattr(exc.orig, 'sqlstate', None) == '23503':
                raise HTTPException(409, 'Packaging type unavailable') from None
            raise
        except ValueError:
            raise HTTPException(400, 'Invalid packaging type input') from None
    return dependency
